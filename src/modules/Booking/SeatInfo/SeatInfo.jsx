...
function SeatInfo({ bookingId }) {
  const { allSeats, selectedSeats, checkoutSeats, isLoading } = useSelector(
    (state) => state.booking
  );
  ...
  const renderSeats = () => {
    return allSeats?.danhSachGhe?.map((seat, index) => {
      ...
      let btnHoverClass =
        seat.daDat || checkoutSeatClass !== "" ? "" : "btnHover";

      return (
        <Fragment key={index}>
          <button
            className={`emptySeat ${vipSeatClass} ${reservedSeatClass} ${selectedSeatClass} ${checkoutSeatClass} ${btnHoverClass}`}
